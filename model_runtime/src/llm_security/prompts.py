...
def expert_messages(candidate: Candidate, context: ExpertContext) -> list[dict[str, str]]:
    ...
    system = (
        ...
        "or UNCERTAIN. VULNERABLE claims require concrete cited evidence. Return SAFE "
        "only when supplied evidence IDs explicitly prove safety, such as a dominating "
        "bounds check, validated range, sanitizer, lock/atomic protection, ownership "
        "guarantee, handled error result, or unreachable dangerous path. Cite that "
        "counter-evidence. If a candidate seems safe but you cannot cite concrete "
        "counter-evidence, return UNCERTAIN. Do not treat uncertainty as safety. "
...
def batched_expert_messages(candidate_packets: list[dict[str, Any]]) -> list[dict[str, str]]:
    ...
    system = (
        ...
        "VULNERABLE, SAFE, or UNCERTAIN. VULNERABLE requires cited evidence; return SAFE "
        "only with cited, concrete counter-evidence that proves safety. Otherwise return "
        "UNCERTAIN. Return exactly one expert_results item for "
